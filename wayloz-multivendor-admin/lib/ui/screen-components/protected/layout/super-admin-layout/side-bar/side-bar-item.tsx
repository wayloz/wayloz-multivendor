...
function HoveredSubMenuItem({ icon, text, active }: SubMenuItemProps) {
  return (
    <div
      className={`my-3 rounded-md p-2 ${active ? 'bg-gray-300' : 'hover:bg-indigo-50'
        }`}
    >
          ...
          subMenu?.map((item, index) => {
            ...
            return (
              <li key={index} className="relative">
                {isActive && (
                  <div className="absolute -left-[0.26rem] top-1/2 z-10 h-2 w-2 -translate-y-1/2 transform rounded-full bg-primary-color"></div>
                )}
