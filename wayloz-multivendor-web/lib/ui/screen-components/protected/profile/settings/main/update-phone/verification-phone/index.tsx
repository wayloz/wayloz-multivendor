...
const VerificationPhone = ({
  ...
  return (
    <div className="flex flex-col items-start justify-start w-full h-full px-4 py-6 md:px-8">
      ...
      {/* <CustomButton
    label={"Continue"}
    className="bg-[#b83330] text-white flex items-center justify-center rounded-full p-3 w-full mb-4 h-12 sm:h-14 text-md lg:text-lg sm:text-md font-medium"
    onClick={handleSubmit}
  /> */}
      <button
        onClick={handleSubmit}
        disabled={loading}
        className={`bg-primary-color text-white flex items-center justify-center rounded-full p-3 w-full mb-4 h-12 sm:h-14 text-md lg:text-lg sm:text-md font-medium ${loading ? "opacity-70 cursor-not-allowed" : ""
          }`}
      >
        {loading ? (
          <FontAwesomeIcon
